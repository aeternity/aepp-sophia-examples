...
describe('Crypto Hamster', async () => {
    ...
    beforeEach(async () => {
        ...
        try {
            ...
            cryptoHamsterInstance = await aeSdk.initializeContract({ sourceCode: contractContent });
            await cryptoHamsterInstance.init();
        } catch(err) {
            console.error(err);
    ...
    it('Should create hamster successfully', async () => {
        await cryptoHamsterInstance.create_hamster(hamsterNames[0]);
        const result = await cryptoHamsterInstance.name_exists(hamsterNames[0]);
    ...
    it('Should create few hamsters successfully', async () => {
        for (let i = 0; i < hamsterNames.length; i++) {
            await cryptoHamsterInstance.create_hamster(hamsterNames[i]);
            const result = await cryptoHamsterInstance.name_exists(hamsterNames[i]);
            assert.isTrue(result.decodedResult, 'Hamster does not exist!');
        }
    });
    ...
    it('[NEGATIVE] Should NOT create hamster with same name', async () => {
        await cryptoHamsterInstance.create_hamster(hamsterNames[0]);
        await assertNode.rejects(cryptoHamsterInstance.create_hamster(hamsterNames[0]), (err) => {
    ...
    it('[NEGATIVE] Should throw exception when there are not any hamsters', async () => {
        for (let i = 0; i < hamsterNames.length; i++) {
            await assertNode.rejects(cryptoHamsterInstance.get_hamster_dna(hamsterNames[i]), (err) => {
    ...
    it('Hamsters DNA should not match', async () => {
        ...
        for (let i = 0; i < hamsterNames.length; i++) {
            await cryptoHamsterInstance.create_hamster(hamsterNames[i]);
            const result = await cryptoHamsterInstance.get_hamster_dna(hamsterNames[i]);
